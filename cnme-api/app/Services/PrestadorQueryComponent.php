...
class PrestadorQueryComponent{
    public function queryPrestadoras($etapa){
        ...
        avg(CASE 
            WHEN status_tarefa IN ('ATRASADA', 'CONCLUIDA COM ATRASO') THEN t2.atraso_medio ELSE 0 END) as atraso_medio
        FROM
        (
        SELECT 
        unidade_responsavel_id,
        ...
        CASE WHEN status_tarefa = 'CONCLUIDA COM ATRASO' THEN COUNT(tarefa_id) END as total_concluida_atrasada, 
            ...
            avg(
                cast(NULLIF(dias_atrasos, 0) AS BIGINT)
    ...
    public function queryPrestadoraPorEstado($etapa,$empresaId){
        ...
        avg(t2.prazo_medio) as prazo_medio,
        avg(t2.periodo_medio) as periodo_medio,
        avg(CASE WHEN status_tarefa IN ('ATRASADA', 'CONCLUIDA COM ATRASO') THEN t2.atraso_medio ELSE 0 END) as atraso_medio
        FROM
        (
        SELECT 
        unidade_responsavel_id,
        ...
        CASE WHEN status_tarefa = 'CONCLUIDA COM ATRASO' THEN COUNT(tarefa_id) END as total_concluida_atrasada, 
            ...
            avg(prazo_total) as prazo_medio,
            avg(periodo_total) as periodo_medio,
            avg(cast(NULLIF( dias_atrasos , 0) AS BIGINT)
    ...
    public function queryPrestadorExtrato($empresaId){
        $result = DB::select(
        "SELECT 
            unidade_responsavel_id,
            empresa, 
    ...
    public function queryPrestadorEtapaExtrato($etapa, $empresaId){
        $result = DB::select(
        "SELECT 
            unidade_responsavel_id,
            empresa, 
